  ...
  <div>
    ...
    <div class="split left">
      <div>
        ...
        <h2>
          <span class="marked-text"> haben geheime Dinge, </span>
          die nach dem Tod verschwinden sollen
        </h2>
      </div>
      <div>
        <h3>
          Hast <span class="marked-text"> du </span> auch geheime Dinge die
          ungesehen verschwinden sollen?
        </h3>
      </div>
    </div>
    <div class="split right">
      <div class="circle-row" v-for="row in Math.ceil(100 / 10)" :key="row">
        <div
          ...
          :key="index"
          :class="{
            'green-circle': (row - 1) * 10 + index <= report['F2.1'],
            'blue-circle': (row - 1) * 10 + index > report['F2.1'],
          }"
          :style="{
...
onMounted(async () => {
  try {
    ...
    report.value = responseData.reports.report08;
  } catch (error) {
